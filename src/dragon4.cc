...
static inline int Min(int x, int y) { return y < x ? y : x; }
static inline int Max(int x, int y) { return y < x ? x : y; }

static inline void Zeroize(uint32_t* dst, int n)
{
    DRAGON4_ASSERT(n >= 0);
    std::memset(dst, 0, static_cast<unsigned>(n) * sizeof(uint32_t));
}

static inline void Memmove(uint32_t* dst, const uint32_t* src, int n)
{
    DRAGON4_ASSERT(n >= 0);
...
static inline void AssignU32(DiyInt& x, uint32_t value)
{
    x.bigits[0] = value;
    x.size = (value != 0) ? 1 : 0;
}
...
// x := A * x + B
static inline void MulAddU32(DiyInt& x, uint32_t A, uint32_t B = 0)
{
...
// x := x * 2^e2
static inline void MulPow2(DiyInt& x, int e2) // aka left-shift
{
    DRAGON4_ASSERT(x.size >= 0);
    DRAGON4_ASSERT(e2 >= 0);
...
// x := x * 5^e5
static inline void MulPow5(DiyInt& x, int e5)
{
    // TODO:
...
static inline void Mul2(DiyInt& x)
{
    uint32_t carry = 0;
    for (int i = 0; i < x.size; ++i)
    {
...
static inline void Mul10(DiyInt& x)
{
#if 0
    MulAddU32(x, 10, 0);
#else
...
// x := 2^e2
static inline void AssignPow2(DiyInt& x, int e2)
{
    DRAGON4_ASSERT(e2 >= 0);
...
// x := 5^e5
static inline void AssignPow5(DiyInt& x, int e5)
{
    // TODO:
    // Optimize?!
...
// x := 10^e10
static inline void AssignPow10(DiyInt& x, int e10)
{
    // TODO:
    // Optimize?!
...
// x := value * 2^e2
static inline void AssignU64MulPow2(DiyInt& x, uint64_t value, int e2)
{
    DRAGON4_ASSERT(e2 >= 0);
...
// x := value * 10^e10
static inline void AssignU64MulPow10(DiyInt& x, uint64_t value, int e10)
{
...
// x := 2^e2 * 5^e5
static inline void AssignPow2MulPow5(DiyInt& x, int e2, int e5)
{
    AssignPow2(x, e2);
    MulPow5(x, e5);
...
static inline uint32_t DivModShort(DiyInt& u, uint32_t v)
{
    const int m = u.size;
...
// return q
// PRE: 0 <= q <= 9
static inline uint32_t DivMod(DiyInt& u, const DiyInt& v)
{
...
static inline int CompareAdd(const DiyInt& a, const DiyInt& b, const DiyInt& c)
{
    // NB:
...
static inline int EffectivePrecision(uint64_t f)
{
    DRAGON4_ASSERT(f != 0);
...
static inline int ComputeInitialValuesAndEstimate(DiyInt& r, DiyInt& s, DiyInt& delta, uint64_t f, int e, bool lower_boundary_is_closer)
{
...
void dragon4::Dragon4(uint64_t& digits, int& exponent, uint64_t f, int e, bool accept_bounds, bool lower_boundary_is_closer)
{
    DiyInt r;
    DiyInt s;
    DiyInt delta;
